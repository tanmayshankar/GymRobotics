...
class ActorModel():
	...
	def __init__(self, input_dimensions, output_dimensions, num_layers=3, hidden_units=10, name_scope=None, env=None):
		self.input_dimensions = input_dimensions
		self.output_dimensions = output_dimensions
		self.num_layers = num_layers
		self.hu_num = hidden_units
		self.env = env
		if name_scope:
	...
	def define_base_model(self, sess, to_train=None):
		...
		self.hidden_units = self.hu_num*npy.ones((self.num_layers+1),dtype=int)        
	...
	def define_actor_layers(self):
		...
		#Factor by which to scale actions. 
		if self.env=='InvertedPendulum-v2':
			self.action_scaling = 3.
		else:
...
class CriticModel():
	...
	def __init__(self, input_dimensions, output_dimensions, num_layers=3, hidden_units=10,name_scope=None):
...
class ActorCriticModel():
	...
	def __init__(self, input_dimensions, output_dimensions, sess=None, number_layers=3, hidden_units=10,to_train=True, env=None):

		self.sess = sess
		self.to_train = to_train		
		# Here we instantiate the actor and critic (don't inherit).

		self.actor_network = ActorModel(input_dimensions,output_dimensions,num_layers=number_layers,hidden_units=hidden_units, name_scope='ActorModel',env=env)
		self.actor_network.define_actor_model(sess,to_train=to_train)		

		self.critic_network = CriticModel(input_dimensions,output_dimensions,num_layers=number_layers,hidden_units=hidden_units,name_scope='CriticModel')
	...
	def define_logging_ops(self):
		if self.to_train:
			...
			self.predicted_action_summary = tf.summary.scalar('Predicted_Action',tf.reduce_mean(self.actor_network.predicted_action))
			self.predicted_Qvalue_summary = tf.summary.scalar('Predicted_Qvalue',tf.reduce_mean(self.critic_network.predicted_Qvalue))
			# self.actor_grads_summary = tf.summary.scalar('Actor_Gradients',tf.reduce_mean(self.actor_gradients))
			# self.critic_grads_summary = tf.summary.scalar('Critic_Gradients',tf.reduce_mean(self.critic_gradients))
