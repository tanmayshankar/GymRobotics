...
class DAggerPolicy():
	...
	def define_policy_layers(self):
		...
		# Here we need a stochastic policy. 
		self.predicted_mean = tf.layers.dense(self.hidden_layers[-1],self.output_dimensions,activation=tf.nn.tanh, name='predicted_mean')
		self.predicted_var = tf.layers.dense(self.hidden_layers[-1],self.output_dimensions,activation=tf.nn.softplus, name='predicted_var')		
	
		self.action_distribution = tfp.distributions.MultivariateNormalDiag(loc=self.predicted_mean,scale_diag=self.predicted_var,name='action_distribution')
		# self.action_distribution = tf.distributions.Normal(loc=self.predicted_mean,scale=self.predicted_var,name='action_distribution')
		self.sample_action = self.action_distribution.sample(name='sample_action')		
		self.sampled_action = tf.placeholder(tf.float32,shape=(None,self.output_dimensions),name='sampled_action')
		self.action_loglikelihood = self.action_distribution.log_prob(self.sampled_action,name='action_loglikelihood')
		self.action_likelihood = self.action_distribution.prob(self.sampled_action,name='action_likelihood')
	...
	def define_actor_training_ops(self):
		...
		self.likelihood_ratio_weight = tf.placeholder(tf.float32,shape=(None,1),name='likelihood_ratio_weight')
		self.actor_loss = -tf.multiply(self.action_loglikelihood,self.likelihood_ratio_weight)
